...
class MicroPythonProxy(BackendProxy):
    def __init__(self, clean):
        ...
        if self._serial is not None and (clean or _AUTOMATIC_INTERRUPT):
            try:
                ...
                self._show_error_connect_again("Could not connect to REPL.\n"
    ...
    def send_command(self, cmd):
        ...
        if cmd.name in ["editor_autocomplete", "cd", "dump_api_info",
                           "lsdevice"]:
            ...
            try:
                if not self._serial.buffers_are_empty():
                    ...
                    self._send_error_to_shell("Warning: when issuing %r,\nincoming was not emtpy: %r"
                                              % (cmd, discarded))
                return super().send_command(cmd)
    ...
    def disconnect(self):
        if self._serial is not None:
            try:
                self._serial.close()
                self._send_text_to_shell("\n\nConnection closed.\nSelect Run → Stop/Restart or press Ctrl+F2 to connect again.", "stdout")
    ...
    def _create_serial(self):
        ...
        if self.port is None:
            self._send_text_to_shell('Not connected. Choose "Tools → Options → Backend" to change.', "stdout")
            return None
        ...
        try:
            ...
            traceback.print_exc()
            message = ("Unable to connect to " + self.port + "\n"
                       + "Error: " + str(error))
    ...
    def _fetch_uname(self):
        assert self.idle
        res = self._execute_and_parse_value("import os as __os_; print(repr(tuple(__os_.uname()))); del __os_")
        return {
            "sysname" : res[0],
            "nodename" : res[1],
            "release" : res[2],
    ...
    def _interrupt_to_prompt(self, clean, timeout=8):
        ...
        for delay in [0.05, 0.5, 2.0, 3.0]:
            ...
            if (self._discarded_bytes.endswith(FIRST_RAW_PROMPT)
        ...
        # report ready
        self._non_serial_msg_queue.put(ToplevelResponse(welcome_text=welcome_text.strip()))
    ...
    def _clean_environment_during_startup(self, time_left):
        ...
        self._discarded_bytes = self._serial.read_until([FIRST_RAW_PROMPT, RAW_PROMPT],
                                                        time_left)
    ...
    def _get_welcome_text_in_raw_mode(self, timeout):
        ...
        welcome_text = self._serial.read_until(NORMAL_PROMPT, timer.time_left).strip(b"\r\n >").decode("utf-8", 'replace')
        if os.name != "nt":
    ...
    def _clear_environment(self):
        ...
        #""").strip())
        #or 
        out, err = self._execute_and_get_response(dedent("""
            globals().clear()
            __name__ == '__main__'
        """).strip())
    ...
    def _execute_async(self, script):
        ...
        assert ok == b"OK", "Expected OK, got %s, followed by %s" % (ok, self._serial.read_all())
    ...
    def _cmd_execute_source(self, cmd):
        try:
            ...
            self._execute_async("print('\\x04\\x02', [%s for v in [%s]][0])" % (msg_template, cmd.source.strip()))
        except SyntaxError:
    ...
    def _cmd_get_globals(self, cmd):
        ...
        try:
            if cmd.module_name == "__main__":
                self._execute_async("print('\\x04\\x02', {'message_class' : 'InlineResponse', 'command_name':'get_globals', 'module_name' : '__main__', 'globals':{x:repr(globals()[x]) for x in globals() if not x.startswith('__')}})")
            else:
                self._execute_async(dedent("""
                    try:
            ...
            self._non_serial_msg_queue.put(InlineResponse(command_name="get_globals", 
    ...
    def _cmd_editor_autocomplete(self, cmd):
        ...
        msg = InlineResponse(command_name="editor_autocomplete",
                             source=cmd.source, 
                             row=cmd.row,
                             column=cmd.column,
                             error=None)
        ...
        try:
            script = jedi.Script(cmd.source, cmd.row, cmd.column, sys_path=[self._get_api_stubs_path()])
    ...
    def _cmd_shell_autocomplete(self, cmd):
        ...
        if source.strip().startswith("import ") or source.strip().startswith("from "):
            ...
            msg = InlineResponse(command_name="shell_autocomplete",
                                 source=cmd.source,
                                 error=None)
            ...
            try:
                ...
                lines = source.split("\n")
                script = jedi.Script(source, len(lines), len(lines[-1]), sys_path=[self._get_api_stubs_path()])
                completions = script.completions()
                msg['completions'] = self.filter_completions(completions)
            except Exception:
                msg['error'] = "Autocomplete error"
    ...
    def _cmd_dump_api_info(self, cmd):
        ...
        try:
            self._execute_and_expect_empty_response(dedent("""
            ...
            for module_name in sorted(self._fetch_builtin_modules()):
                if (not module_name.startswith("_") 
                    ...
                    file_name = os.path.join(self._get_api_stubs_path(), module_name.replace(".", "/") + ".py")
    ...
    def _dump_object_stubs(self, fp, object_expr, indent):
        ...
        items, errors = self._execute_and_parse_value("__print_object_atts({0})".format(object_expr))
        ...
        if target.endswith(".py"):
            with tokenize.open(source) as fp:
                ...
                try:
                    ...
                    self._send_error_to_shell("%s has syntax errors:\n%s\n\nFile will not be uploaded." % (source, e))
                    return
        try:
    ...
    def _report_upload_via_mount_error(self, source, target, error):
        self._send_error_to_shell("Couldn't write to %s\nOriginal error: %s\n\nIf the target directory exists then it may be corrupted." % (target, error))
    ...
    def _list_files(self):
        mount = self._get_fs_mount()
        if mount is None:
            return self._execute_and_parse_value("import os as __os_; print(__os_.listdir()); del __os_")
        else:
    ...
    def _read_next_serial_message(self) -> Optional[MessageFromBackend]:
        ...
        match = re.search(b"(" + EOT 
                          + b"|" + NORMAL_PROMPT + b"$" # Consider prompts only if they're at the end of output
                          + b"|" + FIRST_RAW_PROMPT + b"$"
                          + b")", new_bytes)
                ...
                if next_eot_loc == -1:
                    ...
                    if len(block_bytes) > 0:
                        ...
                        return BackendEvent(
                            ...
                            stream_name="stderr",
                            data=self.transform_output(block_bytes.decode("utf-8", 'replace'), "stderr")
                        )
                    else:
                        return None
    ...
    def transform_output(self, s, stream_name):
        ...
        return s.replace("Press any key to enter the REPL. Use CTRL-D to reload.",
    ...
    def _enter_raw_repl(self, strict):
        ...
        self._send_error_to_shell("Couldn't connect to the raw REPL. Serial output: " 
                                  + str(discarded_data))
    ...
    def _send_text_to_shell(self, message_text, stream_name):
        ...
        self._non_serial_msg_queue.put(BackendEvent(
                                        event_type="ProgramOutput",
...
class MicroPythonConfigPage(BackendDetailsConfigPage):
    ...
    def __init__(self, master):
        ...
        self._ports_by_desc = {p.description if p.device in p.description 
        ...
        self._port_combo = ttk.Combobox(self,
...
def load_plugin():
    add_micropython_backend("GenericMicroPython", GenericMicroPythonProxy, 
                            "MicroPython on a generic device", GenericMicroPythonConfigPage)
    ...
    get_workbench().add_command("uploadmainscript", "device", "Upload current script as main script",
    ...
    get_workbench().add_command("catmainscript", "device", "Show device's main script",
                                _cat_main_script,
                                tester=file_commands_enabled,
                                group=20)

    get_workbench().add_command("catbootscript", "device", "Show device's boot script",
                                _cat_boot_script,
                                tester=file_commands_enabled,
                                group=20)

    get_workbench().add_command("disconnectserial", "device", "Close serial connection",
                                disconnect,
                                disconnect_enabled,
                                group=100,)
