...
PREFIX=$HOME/thonny_template_build_39
...
PYTHON_CURRENT=$FRAMEWORKS/Python.framework/Versions/3.9

# install #####################################################
export MACOSX_DEPLOYMENT_TARGET=10.9
export SDKROOT=~/MacOSX10.9.sdk

$PYTHON_CURRENT/bin/python3.9 -s -m pip install --no-cache-dir wheel

for req_file in $req_files
do
	echo "installing from $req_file"
	$PYTHON_CURRENT/bin/python3.9  -s -m pip install --no-binary mypy --no-binary lxml -r $req_file
done
$PYTHON_CURRENT/bin/python3.9 -s -m pip install --no-cache-dir certifi
$PYTHON_CURRENT/bin/python3.9 -s -m pip install --pre --no-cache-dir "thonny==${VERSION}"
...
rm -rf $PYTHON_CURRENT/lib/python3.9/test
rm -rf $PYTHON_CURRENT/lib/python3.9/distutils/test
rm -rf $PYTHON_CURRENT/lib/python3.9/lib2to3/test
rm -rf $PYTHON_CURRENT/lib/python3.9/unittest/test

rm -rf $PYTHON_CURRENT/lib/python3.9/idlelib
rm -rf $PYTHON_CURRENT/bin/idle3
rm -rf $PYTHON_CURRENT/bin/idle3.9

rm -rf $PYTHON_CURRENT/lib/python3.9/site-packages/pylint/test
rm -rf $PYTHON_CURRENT/lib/python3.9/site-packages/mypy/test
...
find $PYTHON_CURRENT/lib/python3.9/site-packages/lxml -name '*.txt' -delete       || true
find $PYTHON_CURRENT/lib/python3.9/site-packages/matplotlib -name '*.txt' -delete || true


# create link to Python.app interpreter
cd build/Thonny.app/Contents/MacOS
ln -s ../Frameworks/Python.framework/Versions/3.9/Resources/Python.app/Contents/MacOS/Python Python
cd $SCRIPT_DIR
