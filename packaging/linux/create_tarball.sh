...
#    $TARGET_DIR/bin/python3.7 -s -m pip install setuptools-scm
    ...
    $TARGET_DIR/bin/python3.7 -s -m pip install cryptography==3.2.*
fi
...
if [ `getconf LONG_BIT` = "32" ]
then
  cp /usr/lib/i386-linux-gnu/libffi.so.6.0.4 $TARGET_DIR/bin
else
  cp /usr/lib/x86_64-linux-gnu/libffi.so.6.0.4 $TARGET_DIR/bin
fi
cd $TARGET_DIR/lib
